...
int MPU6050_Init(inv_mpu6050_handle this, inv_imu_config _cfg) {
    ...
    //打开所有传感器
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_2, 0);

    //1khz采样率
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_SMPLRT_DIV, 0);

    //配置陀螺仪lpf
    _InvGetMapVal(MPU6050_GBW_MAP, this->parents.cfg.gyroBandwidth, bw);
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_CONFIG, bw);
    ...
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_GYRO_CONFIG, fs << 3u);
    ...
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_ACCEL_CONFIG, fs << 3u);
...
bool MPU6050_Detect(inv_mpu6050_handle this) {
    ...
    IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_WHO_AM_I, &val);
    if (0x68 == val) {
        return true;
    }
    val = 0;
...
int MPU6050_SelfTest(inv_mpu6050_handle this) {
    ...
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_GYRO_CONFIG, &val);
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_GYRO_CONFIG, val | (0b111 << 5));//打开陀螺仪自检
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_ACCEL_CONFIG, &val);
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_ACCEL_CONFIG, val | (0b111 << 5));//打开加速度计自检
    times = 20;
    while (times--) { while (!IMU_DataReady((inv_imu_handle)this)) {}}//丢弃前100个数据
    times = 20;
    while (times--) {
        while (!IMU_DataReady((inv_imu_handle)this)) {}
        res |= IMU_ReadSensorBlocking((inv_imu_handle)this);
    ...
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_SELF_TEST_X, regs);
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_SELF_TEST_Y, regs + 1);
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_SELF_TEST_Z, regs + 2);
    res |= IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_SELF_TEST_A, regs + 3);
    int a_st[3];
    int g_st[3];
    int ft_a[3];
    int ft_g[3];
...
bool MPU6050_DataReady(inv_mpu6050_handle this) {
    uint8_t val = 0;
    IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_INT_STATUS, &val);
    return (val & 0x01) == 0x01;
}
int MPU6050_EnableDataReadyInt(inv_mpu6050_handle this) {
    return IMU_ModifyReg((inv_imu_handle)this, (uint8_t) MPU6050_INT_ENABLE, 0x01, 0x01);
}
int MPU6050_SoftReset(inv_mpu6050_handle this) {
    ...
    uint8_t val;
    //复位
    res |= IMU_WriteReg((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_1, 0x80);
    //等待复位成功
    do {
        IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_1, &val);
    ...
    IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_1, &val);
    IMU_ReadReg((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_1, &val);
    res |= IMU_WriteRegVerified((inv_imu_handle)this, (uint8_t) MPU6050_PWR_MGMT_1, 0x0);

    return res;
}
int MPU6050_ReadSensorBlocking(inv_mpu6050_handle this) {
    this->parents.i2cTransfer.direction = inv_i2c_direction_Read;
    this->parents.i2cTransfer.subAddress = (uint8_t) MPU6050_ACCEL_XOUT_H;
    this->parents.i2cTransfer.data = this->buf;
    this->parents.i2cTransfer.dataSize = 14;
    return this->parents.i2c.masterTransferBlocking(&this->parents.i2cTransfer);
}
int MPU6050_ReadSensorNonBlocking(inv_mpu6050_handle this) {
    ...
    this->parents.i2cTransfer.data = this->buf;
    this->parents.i2cTransfer.dataSize = 14;
    return this->parents.i2c.masterTransferNonBlocking(&this->parents.i2cTransfer);
}
int MPU6050_Convert(inv_mpu6050_handle this, float *array) {
...
int MPU6050_Convert2(inv_mpu6050_handle this, int16_t *raw) {
    uint8_t *buf = this->buf;
...
int MPU6050_Convert3(inv_mpu6050_handle this, float *temp) {
    if (temp) {
        *temp = (float) ((int16_t) ((((int16_t)this->buf[6] << 8) | this->buf[7]) - 521)) / 340.0f + 35;
    }
    return 0;
}
